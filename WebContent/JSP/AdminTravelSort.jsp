...
<body style="background: linear-gradient(to right, #efefbb, #d4d3dd);">	
	<div id="loading" style="width: 100%; height: 150%; position: absolute; background-color: black; margin: auto; text-align: center;">
		...
		<h2 style="text-align: center; color: white">Fetching journey details from database!</h2>
	</div>
			...
			<table id="customers">
				...
				<%
					...
					{     
						...
						if (Option.equals("PN")) {
							Ps=Con.prepareStatement("SELECT *FROM journey ORDER BY PName");
						} else if (Option.equals("DN")) {
							Ps=Con.prepareStatement("SELECT *FROM journey ORDER BY DName");
						} else if (Option.equals("Dist")) {
							Ps=Con.prepareStatement("SELECT *FROM journey ORDER BY Distance");
						} else {
							Ps=Con.prepareStatement("SELECT *FROM journey ORDER BY Rating DESC");
						}
						
						ResultSet Rs=Ps.executeQuery();
						
						if (Rs==null) {
							%><tr><td colspan='14' align='center'>No journeys yet!<td></tr><%
						} else {
							int num=1;
					    	while(Rs.next()) {
					    		%><tr>
