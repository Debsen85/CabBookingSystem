...
public class SuccessDao extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			Ps.setString(12, req.getParameter("star"));
			Ps.setString(13, "Successful");
			
			Ps.executeUpdate();
			
			PreparedStatement PsNext=con.prepareStatement("SELECT AVG(Rating) as AvgRating FROM journey WHERE DriverPhone=?");
			
			PsNext.setString(1, DriverPhone);
			ResultSet Rs=PsNext.executeQuery();
			
			if (Rs.next()) {
				double Rate=Rs.getDouble("AvgRating");
				
				PreparedStatement PsAfter=con.prepareStatement("UPDATE driver SET Rating=? WHERE DriverPhone=?");
				
				PsAfter.setString(1, Double.toString(Rate));
				PsAfter.setString(2, DriverPhone);
				
				PsAfter.executeUpdate();
			}
			
			RequestDispatcher rd=req.getRequestDispatcher("/JSP/CabBookSuccess.jsp");
			rd.forward(req, resp);			
